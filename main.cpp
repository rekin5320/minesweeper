...
public:
    ...
    void create_tiles(Board& board) {
        for (auto& tile : board.Tiles) {
            ...
            QObject::connect(tile.button.get(), &MyButton::leftButtonClicked, [&board, x, y](){board.uncover_tile(x, y);});
            QObject::connect(tile.button.get(), &MyButton::rightButtonClicked, [&board, x, y](){board.flag_tile(x, y);});
        }
    };
};
